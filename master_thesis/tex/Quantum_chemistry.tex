...
the approximation. A method used for this is called the \ac{HF} \ac{SCF} method.
...
Each one electron Hamiltonian $h_i$ has a corresponding one electron \ac{SE} with
...
\end{equation}
and then the total \ac{SE}  will have the form \cite{Cramer:2004}
\begin{align}
  \begin{split}
...
We now will attempt to find appropiate wave functions using the \ac{HF} method.
...
\cite{Cramer:2004, Helgaker:2012}. We set a secular equation as follows
\begin{equation}
  \begin{vmatrix}
  ...
  \end{vmatrix} = 0
\end{equation}
Where the matrix elements of $\bar{S}$ are defined as on Equation \ref{eq:seceq}. The
matrix elements of $\bar{F}$ are computed as follows \cite{Cramer:2004}
\begin{equation}
...
functions $\phi$ and the numerical subscripts to reference to single electrons.
