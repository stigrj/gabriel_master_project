...
\begin{equation}\label{eq:totalheliumSE}
  \hat{H}\Psi(\vec{R}, \rvec) = E \Psi(\vec{R}, \rvec)
\end{equation}
...
\begin{equation}
  \hat{H}(\vec{R}, \rvec) = \hat{T}_e + \hat{T}_N + \hat{V}_{Ne} +\hat{V}_{NN} + \hat{V}_{ee}
\end{equation}
...
kinetic energies and $N\cdotn$ terms in the attraction potential $\hat{V}_Ne$}.
...
the nuclei themselves are moving with respect to each other \.
