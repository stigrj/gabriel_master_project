...
I've yet to implement this dielectric function. I will work with the method used in \cite{FossoTande:2013ka} in order to create this cavity. My implementation right now has the cavity function and the electron density as two separate independent functions. What i need to do is to make the dielectric function (the function dependen on the cavity function) a functional dependent on $\rho(\bar{r})$ where $\bar{r}$ is a vector of three dimensional coordinates.
